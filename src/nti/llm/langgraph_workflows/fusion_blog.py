...
def generate_draft(state: DraftState) -> dict:
    ...
    logger.info(f"Generating draft with provider: {provider.name} (model: {provider.model})")
...
def critique_draft(state: CritiqueState) -> dict:
    ...
    logger.info(f"Critique: {critic_provider.name} critiquing {draft_provider}'s draft")
